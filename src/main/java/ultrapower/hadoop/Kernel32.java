...
public interface Kernel32 extends StdCallLibrary {
	...
	public class BY_HANDLE_FILE_INFORMATION extends Structure {
		...
		protected List<String> getFieldOrder() {
			...
			order.add("dwFileAttributes");
			order.add("ftCreationTime");
			order.add("ftLastAccessTime");
			order.add("ftLastWriteTime");
			order.add("dwVolumeSerialNumber");
			order.add("nFileSizeHigh");
			order.add("nFileSizeLow");
			order.add("nNumberOfLinks");
			order.add("nFileIndexHigh");
			order.add("nFileIndexLow");
			return order;
		}
